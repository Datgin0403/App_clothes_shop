...
class _OrderConfirmPageState extends State<OrderConfirmPage> {
  ...
  bool _isLoading = false; // Bỏ từ khóa `final` để có thể cập nhật trạng thái
  ...
  Future<void> _createOrder() async {
    ...
    if (_selectedPaymentMethod == 'banking' &&
        (_selectedBank == null || _bankAccountController.text.isEmpty)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Vui lòng chọn ngân hàng và nhập số tài khoản'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    if (_useNewAddress) {
      ...
      if (address == null && _addresses.isNotEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Vui lòng chọn hoặc nhập địa chỉ giao hàng'),
            backgroundColor: Colors.red,
          ),
        );
        return;
    ...
    setState(() {
      _isLoading = true; // Bật trạng thái đang tải
    });

    try {
      print("Giỏ hàng có ${widget.cartItems.length} sản phẩm");
      ...
      for (var item in widget.cartItems) {
        if (item.SKU == null || item.quantity == null) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Thông tin sản phẩm không hợp lệ (thiếu SKU hoặc số lượng)'),
              backgroundColor: Colors.red,
            ),
          );
          setState(() {
      ...
      if (success) {
        ...
        // Chuyển hướng đến OrderListScreen
      ...
      setState(() {
        _isLoading = false; // Tắt trạng thái đang tải nếu có lỗi
      });
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Lỗi: $e'),
